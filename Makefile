...
help:
	...
	@echo "  test-integration   Run integration tests (requires cluster)"
	@echo "  test-integration-create  Create cluster and run integration tests"
	...
	@echo "  run-local          Run NodeReaper locally with debug logging"
	@echo "  run-local-json     Run NodeReaper locally with JSON logging"
	@echo "  run-local-prod     Run NodeReaper locally with production settings"
	@echo ""
	@echo "Docker:"
...
helm-install-prod:
	helm upgrade --install nodereaper $(HELM_CHART) \
		...
		--set config.deletionTimeout=10m \
		--set config.enableJsonLogs=true \
...
.PHONY: run-local
run-local:
	DRY_RUN=true LOG_LEVEL=DEBUG ENABLE_JSON_LOGS=false nodereaper

.PHONY: run-local-json
run-local-json:
	DRY_RUN=true LOG_LEVEL=DEBUG ENABLE_JSON_LOGS=true nodereaper

.PHONY: run-local-prod
run-local-prod:
	LOG_LEVEL=INFO ENABLE_JSON_LOGS=true nodereaper
